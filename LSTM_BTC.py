...
filename = '/Users/paulbergeon/Documents/mémoireESSCA/PrixBTC.csv'
...
def create_dataset(dataset, look_back=1):
    ...
    for i in range(len(dataset) - look_back - 1):
        ...
        X.append(a)
        Y.append(dataset[i + look_back, 0])  # Utilisez l'indice 1 pour 'Close normalisé'
    return np.array(X), np.array(Y)


look_back = 100
train_X, train_Y = create_dataset(train_data[['Close normalisé']].values, look_back)
test_X, test_Y = create_dataset(test_data[[ 'Close normalisé' ]].values, look_back)

print("Taille de train_X:", train_X.shape)
print("Taille de test_X:", test_X.shape)

model = Sequential()
model.add(LSTM(300, input_shape=(look_back, 1)))
model.add(Dense(1))
