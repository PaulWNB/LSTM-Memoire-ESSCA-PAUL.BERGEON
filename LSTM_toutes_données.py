...
df['Vol normalisé'] = vol_scaler.fit_transform(df['Vol'].values.reshape(-1, 1))

haut_scaler = MinMaxScaler(feature_range=(0, 1))
df['Haut normalisé'] = haut_scaler.fit_transform(df['Haut'].values.reshape(-1, 1))

bas_scaler = MinMaxScaler(feature_range=(0, 1))
df['Bas normalisé'] = bas_scaler.fit_transform(df['Bas'].values.reshape(-1, 1))

volatilite_scaler = MinMaxScaler(feature_range=(0, 1))
df['Volatilite normalisée'] = volatilite_scaler.fit_transform(df['Volatilite'].values.reshape(-1, 1))
...
look_back = 100
train_X, train_Y = create_dataset(train_data[['Sentiment normalisé', 'Close normalisé', 'Variation normalisée', 'ETH normalisé', 'Vol normalisé', 'Haut normalisé', 'Bas normalisé', 'Volatilite normalisée']].values, look_back)
test_X, test_Y = create_dataset(test_data[['Sentiment normalisé', 'Close normalisé', 'Variation normalisée', 'ETH normalisé', 'Vol normalisé', 'Haut normalisé', 'Bas normalisé', 'Volatilite normalisée']].values, look_back)

print("Taille de train_X:", train_X.shape)
print("Taille de test_X:", test_X.shape)

model = Sequential()
model.add(LSTM(300, input_shape=(look_back, 8)))
model.add(Dense(1))
...
# Créer les séquences d'entrée pour les prédictions futures
future_look_back = 100  # La même valeur que look_back utilisée pour l'entraînement
future_X, _ = create_dataset(data[['Sentiment normalisé', 'Close normalisé', 'Variation normalisée', 'ETH normalisé', 'Vol normalisé', 'Haut normalisé', 'Bas normalisé', 'Volatilite normalisée']].values, future_look_back)

# Prédire les valeurs futures
future_predict = model.predict(future_X)

# Inverser la normalisation des prédictions
future_predict = price_scaler.inverse_transform(future_predict)

# Obtenir les dates correspondantes aux valeurs futures
future_dates = data.iloc[future_look_back + 1:]['Date'].values
